    ...
    def parse(frame: bytes, strict_crc: bool = False) -> M200Response:
        ...
        if crc_received != crc_calculated:
            ...
            if strict_crc:
                ...
                logger.warning(f"{error_msg} - Device may use proprietary CRC variant, continuing anyway")
...
def parse_device_info(data: bytes) -> Dict[str, Any]:
    ...
    if len(data) < EXPECTED_LENGTH:
        logger.warning(f"Device info response shorter than expected: {len(data)} bytes (expected {EXPECTED_LENGTH})")
    ...
    # CPFirmVer (32 bytes)
    cp_fw_ver = data[offset:offset+32].rstrip(b'\x00').decode('ascii', errors='ignore').strip() if len(data) >= offset+32 else ""
    offset += 32
    
    # CPSN_code (12 bytes)
    cp_sn = data[offset:offset+12].rstrip(b'\x00').decode('ascii', errors='ignore').strip() if len(data) >= offset+12 else ""
    offset += 12
    
    # RFIDModeVer (32 bytes)
    rfid_hw_ver = data[offset:offset+32].rstrip(b'\x00').decode('ascii', errors='ignore').strip() if len(data) >= offset+32 else ""
    offset += 32
    
    # RFIDModeName (32 bytes)
    rfid_module_name = data[offset:offset+32].rstrip(b'\x00').decode('ascii', errors='ignore').strip() if len(data) >= offset+32 else ""
    offset += 32
    
    # RFIDMode_SNCode (12 bytes)
    rfid_sn = data[offset:offset+12].rstrip(b'\x00').decode('ascii', errors='ignore').strip() if len(data) >= offset+12 else ""
