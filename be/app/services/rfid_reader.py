    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if buffered:
                    logger.debug(f"Cleared {len(buffered)} bytes of buffered data: {buffered.hex().upper()}")
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if first_chunk[0] != HEAD:
                    logger.warning(f"Unexpected first byte: 0x{first_chunk[0]:02X} (expected 0x{HEAD:02X})")
                ...
                logger.debug(f"← RX: {response.hex().upper()} (len={len(response)}, CMD=0x{response_cmd:04X})")
                ...
                if response_cmd == command.cmd:
                    ...
                    logger.warning(f"Unsolicited message: CMD=0x{response_cmd:04X} (expected 0x{command.cmd:04X}), ignoring and reading next message")
                    continue
        ...
        try:
            ...
            try:
                ...
                if response.cmd != M200Commands.RFM_GET_DEVICE_INFO:
                    logger.warning(f"Response CMD mismatch: expected 0x{M200Commands.RFM_GET_DEVICE_INFO:04X}, got 0x{response.cmd:04X}")
                    logger.warning(f"This might be an unsolicited message or different protocol variant")
                ...
                if response_text.startswith('Content-Length:') or response_text.startswith('HTTP/'):
                    logger.error("⚠️  Protocol mismatch: Reader is responding with JSON/HTTP, not binary M-200 protocol")
                    logger.error(f"This port ({self.reader_port}) appears to be a debug/HTTP port, not the RFID data port")
                        ...
                        if 'Content-Length:' in response_text:
                            ...
                            for part in parts:
                                if part.strip().startswith('{'):
                                    ...
                                    logger.error(f"JSON response type: {json_data.get('type', 'unknown')}")
                                    if json_data.get('type') == 'event' and 'debugpy' in str(json_data):
                                        logger.error("This appears to be a Python debugger port (debugpy), not the RFID reader!")
                    except:
                        pass
                ...
                logger.error(f"Raw response ({len(response_bytes)} bytes): {response_bytes.hex().upper()}")
            ...
            if not response.success:
                logger.error(f"Get device info failed: {M200Status.get_description(response.status)}")
                return {
                    "connected": True,
            ...
            logger.info(f"Device info: {info.get('reader_type')} v{info.get('rfid_firmware_version')}")
            return info
    ...
    async def read_single_tag(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if tags:
                ...
                for tag in tags:
                    logger.debug(f"  Tag: EPC={tag['epc']}, RSSI={tag['rssi']}dBm, Ant={tag['antenna_port']}")
