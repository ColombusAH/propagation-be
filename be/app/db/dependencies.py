...
async def get_db(request: Request) -> Prisma:
    ...
    try:
        ...
        logger.error("State attribute missing, Prisma client likely not initialized.", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Database service initialization error."
        )
    except Exception as e:
        logger.error(f"An unexpected error occurred while getting DB connection: {e}", exc_info=True)
        raise HTTPException(
