...
async def test_list_theft_alerts(client, db_session, override_auth):
    ...
    response = await client.get(
        "/api/v1/alerts/?limit=10", headers={"Authorization": "Bearer token"}
    )
...
async def test_get_my_alerts(client, db_session, override_auth):
    ...
    response = await client.get(
        "/api/v1/alerts/my-alerts", headers={"Authorization": "Bearer token"}
    )
...
async def test_resolve_alert(client, db_session, override_auth):
    ...
    with patch(
        "app.api.v1.endpoints.alerts.theft_service.resolve_alert", new_callable=AsyncMock
    ) as mock_resolve:
...
async def test_mark_alert_read(client, db_session, override_auth):
    ...
    response = await client.post(
        "/api/v1/alerts/mark-read/alert-1", headers={"Authorization": "Bearer token"}
    )
...
async def test_mark_alert_read_not_found(client, db_session, override_auth):
    ...
    response = await client.post(
        "/api/v1/alerts/mark-read/unknown", headers={"Authorization": "Bearer token"}
    )
