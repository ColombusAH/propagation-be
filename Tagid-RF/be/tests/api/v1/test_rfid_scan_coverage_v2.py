...
class TestRfidScanCoverageV2:
    ...
    async def test_perform_inventory_success(self, client, auth_override):
        with (
            ...
            mock_rfid.is_connected = True
            mock_rfid.read_single_tag = AsyncMock(return_value=[
                {"epc": "E1", "rssi": -60},
                {"epc": "E2", "rssi": -55}
            ])
            
            # Mock mapping found for E1, not for E2
            mock_prisma.client.tagmapping.find_unique = AsyncMock(side_effect=[
                MagicMock(encryptedQr="QR_E1"),
                None
            ])
    ...
    async def test_read_tag_memory(self, client, auth_override):
        with patch("app.api.v1.endpoints.rfid_scan.rfid_reader_service") as mock_rfid:
            ...
            response = await client.post("/api/v1/rfid-scan/read-memory", json={"mem_bank": 2, "start_addr": 0, "word_count": 6})
            assert response.status_code == 200
    ...
    async def test_rssi_filter(self, client, auth_override):
        with patch("app.api.v1.endpoints.rfid_scan.rfid_reader_service") as mock_rfid:
            ...
            response = await client.post("/api/v1/rfid-scan/rssi-filter", json={"antenna": 1, "threshold": 50})
            assert response.status_code == 200
    ...
    async def test_gpio_endpoints(self, client, auth_override):
        with patch("app.api.v1.endpoints.rfid_scan.rfid_reader_service") as mock_rfid:
            ...
            response = await client.post("/api/v1/rfid-scan/gpio/config", json={"pin": 1, "direction": 1})
            assert response.status_code == 200
