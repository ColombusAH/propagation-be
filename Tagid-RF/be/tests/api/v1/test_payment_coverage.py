...
class TestCreatePaymentIntent:
    ...
    async def test_create_payment_intent_nexi(self, client):
        ...
        with patch("app.api.v1.endpoints.payment.prisma_client") as mock_prisma, \
            ...
            mock_nexi_instance.create_payment_intent = AsyncMock(return_value={
                "external_id": "nexi-123",
                "client_secret": "secret-abc"
            })
            mock_nexi.return_value = mock_nexi_instance
            
            mock_prisma.client.payment.create = AsyncMock(return_value=MagicMock(
                id="payment-1",
                orderId="order-1",
                amount=1000,
                currency="ILS"
            ...
            response = await client.post("/api/v1/payment/create-intent", json={
                "order_id": "order-1",
                "amount": 1000,
                "currency": "ILS",
    ...
    async def test_create_payment_intent_cash(self, client):
        ...
        with patch("app.api.v1.endpoints.payment.prisma_client") as mock_prisma, \
            ...
            mock_cash = MagicMock()
            mock_cash.create_payment = AsyncMock(return_value=MagicMock(
                success=True,
                payment_id="cash-123"
            ))
...
class TestConfirmPayment:
    ...
    async def test_confirm_payment_success(self, client):
        ...
        with patch("app.api.v1.endpoints.payment.prisma_client") as mock_prisma, \
            ...
            mock_prisma.client.payment.find_unique = AsyncMock(return_value=MagicMock(
                id="payment-1",
                externalId="ext-123",
                provider="NEXI",
                orderId="order-1"
            ...
            mock_gw = MagicMock()
            mock_gw.confirm_payment = AsyncMock(return_value=MagicMock(
                success=True,
                status=MagicMock(value="COMPLETED")
            ))
            mock_gateway.return_value = mock_gw

            response = await client.post("/api/v1/payment/confirm", json={
...
class TestCreateCashPayment:
    ...
    async def test_create_cash_payment_success(self, client):
        ...
        with patch("app.api.v1.endpoints.payment.prisma_client") as mock_prisma, \
            ...
            response = await client.post("/api/v1/payment/cash", json={
                "order_id": "order-1",
                "amount": 500,
                "notes": "Cash payment"
            })
...
class TestRefundPayment:
    ...
    async def test_refund_payment_success(self, client):
        ...
        with patch("app.api.v1.endpoints.payment.prisma_client") as mock_prisma, \
            ...
            mock_gw = MagicMock()
            mock_gw.refund_payment = AsyncMock(return_value=MagicMock(
                success=True,
                refund_id="refund-123"
            ))
            mock_gateway.return_value = mock_gw

            response = await client.post("/api/v1/payment/refund", json={
                "payment_id": "payment-1",
                "amount": 500
            })
