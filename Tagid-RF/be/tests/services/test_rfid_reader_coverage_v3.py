...
async def test_send_command_unsolicited_and_retries(reader):
    ...
    mock_sock.recv.side_effect = [
        build_frame(wrong_cmd)[:1], build_frame(wrong_cmd)[1:6], build_frame(wrong_cmd)[6:], # Wrong CMD
        build_frame(correct_cmd)[:1], build_frame(correct_cmd)[1:6], build_frame(correct_cmd)[6:] # Correct CMD
    ]
...
async def test_process_tag_new_and_existing(reader):
    ...
    with (
        ...
        patch("app.services.rfid_reader.manager.broadcast", new_callable=AsyncMock) as mock_broadcast,
...
async def test_get_reader_info_status_error(reader):
    ...
    with patch.object(reader, "_send_command", return_value=b"raw"), \
...
async def test_get_reader_info_success_parsing(reader):
    ...
    data = b"CP_HW_VER".ljust(32, b"\x00") + \
...
async def test_exhaustive_method_calls(reader):
    ...
    mock_res = MagicMock(success=True, data=b"\x01\x02\x03\x04\x05\x06\x07\x08\x09\x10\x11\x12\x00\x00", status=0x00)
    with patch.object(reader, "_send_command", return_value=b"raw"), \
         patch("app.services.m200_protocol.M200ResponseParser.parse", return_value=mock_res):
        
        assert await reader.initialize_device() is True
        assert await reader.set_power(20) is True
        assert await reader.read_tag_memory(1, 0, 2) == b"\x01\x02\x03\x04\x05\x06\x07\x08\x09\x10\x11\x12\x00\x00"
...
async def test_send_command_unsolicited_and_other_bytes(reader, caplog):
    ...
    mock_sock.recv.side_effect = [
        wrong_frame[0:1], wrong_frame[1:6], wrong_frame[6:8],
        right_frame[0:1], right_frame[1:6], right_frame[6:8]
    ]
    result = reader._send_command(cmd)
