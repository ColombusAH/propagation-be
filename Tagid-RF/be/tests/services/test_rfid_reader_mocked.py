...
class TestRFIDReaderServiceMocked:
    ...
    async def test_connect_success_with_buffer_clear(self, service, mock_socket):
        ...
        with patch('socket.socket', return_value=mock_socket):
            ...
            with patch.object(service, 'get_reader_info', return_value={"connected": True, "serial_number": "TEST1"}):
    ...
    async def test_send_command_unsolicited_handling(self, service, mock_socket):
        ...
        mock_socket.recv.side_effect = [
            b'\xCF', b'\x00\xBB\xBB\x00', b'\x00\x11',
            b'\xCF', b'\x00\xAA\xAA\x00', b'\x00\x22'
        ] + [b'\x00'] * 50
    ...
    async def test_process_tag(self, service):
        ...
        with patch.object(rfid_reader_module, 'SessionLocal', return_value=mock_db), \
             patch.object(rfid_reader_module.manager, 'broadcast', new_callable=AsyncMock) as mock_broadcast:
             ...
             with patch('app.db.prisma.prisma_client') as mock_pc_instance:
                 ...
                 with patch.object(rfid_reader_module, 'RFIDTag') as MockTag, \
