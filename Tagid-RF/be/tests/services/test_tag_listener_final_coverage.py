...
async def test_run_listener_error(service):
    ...
    with patch("app.services.tag_listener_service.start_server", side_effect=Exception("Bind Error")):
...
async def test_broadcast_tag_sqlalchemy_error(service):
    ...
    with patch("app.services.database.SessionLocal", side_effect=Exception("DB Down")), \
         patch("app.routers.websocket.manager.broadcast", new_callable=AsyncMock) as mock_b:
...
def test_start_stop_scan(service):
    ...
    with patch("app.services.tag_listener_service.start_inventory", return_value=True) as m_start, \
