...
async def test_connect_success():
    ...
    with patch("socket.socket") as mock_sock_cls:
        ...
        service.get_reader_info = AsyncMock(return_value={"connected": True, "serial_number": "TEST"})
...
async def test_get_reader_info_success():
    ...
    with patch("app.services.m200_protocol.M200ResponseParser.parse", return_value=mock_response):
        ...
        with patch("app.services.rfid_reader.parse_device_info", return_value={"mock": "data", "fw_version": "1.0"}):
...
async def test_get_network_config():
    ...
    with patch("app.services.m200_protocol.M200ResponseParser.parse", return_value=mock_response):
        with patch("app.services.rfid_reader.parse_network_response", return_value={"ip": "1.2.3.4"}):
...
async def test_read_single_tag_success():
    ...
    with patch("app.services.m200_protocol.M200ResponseParser.parse", return_value=mock_response):
        with patch("app.services.rfid_reader.parse_inventory_response", return_value=[{"epc": "E200", "rssi": -50, "antenna_port": 1}]):
