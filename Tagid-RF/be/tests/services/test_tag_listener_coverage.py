...
async def test_broadcast_tag_success_with_encryption(service):
    ...
    with patch("app.db.prisma.prisma_client") as mock_p:
        ...
        with patch("app.services.database.SessionLocal") as mock_session_cls:
            ...
            with patch("app.services.tag_encryption.get_encryption_service", return_value=mock_encrypt_svc):
                with patch("app.services.tag_listener_service.manager.broadcast", new_callable=AsyncMock) as mock_broadcast:
