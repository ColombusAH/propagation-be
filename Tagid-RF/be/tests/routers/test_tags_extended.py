...
async def test_reader_connect_success(client: AsyncClient):
    ...
    with patch("app.services.rfid_reader.rfid_reader_service.connect", return_value=True), \
         patch("app.services.rfid_reader.rfid_reader_service.get_reader_info", new_callable=AsyncMock, return_value={"model": "MOCK"}):
...
async def test_reader_read_single_success(client: AsyncClient):
    ...
    with patch("app.services.rfid_reader.rfid_reader_service.is_connected", True), \
         patch("app.services.rfid_reader.rfid_reader_service.read_single_tag", new_callable=AsyncMock, return_value=mock_tag):
...
async def test_reader_stop_scanning_success(client: AsyncClient):
    ...
    with patch("app.services.rfid_reader.rfid_reader_service.stop_scanning", new_callable=AsyncMock):
...
async def test_get_reader_status_connected(client: AsyncClient):
    ...
    with patch("app.services.rfid_reader.rfid_reader_service.get_reader_info", new_callable=AsyncMock, return_value=mock_info), \
...
async def test_reader_connect_failure(client: AsyncClient):
    ...
    with patch("app.services.rfid_reader.rfid_reader_service.connect", new_callable=AsyncMock, return_value=False):
        response = await client.post("/api/v1/tags/reader/connect")
...
async def test_live_recent_not_running(client: AsyncClient):
    ...
    with patch("builtins.__import__", side_effect=mocked_import):
        ...
        assert "not available" in response.json()["message"] or "not running" in response.json()["message"]
