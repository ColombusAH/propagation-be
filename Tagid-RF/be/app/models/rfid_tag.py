...
class RFIDTag(Base):
    ...
    epc = Column(
        String(128), unique=True, nullable=False, index=True, comment="Electronic Product Code"
    )
...
class RFIDScanHistory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    epc = Column(
        String(128), nullable=False, index=True, comment="Tag EPC (no foreign key constraint)"
    )
