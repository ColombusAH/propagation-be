...
async def login_with_email(credentials: UserLogin, db: Prisma = Depends(get_db)) -> TokenResponse:
    ...
    try:
        ...
        return TokenResponse(
            message="Login successful", user_id=user.id, role=str(user.role), token=access_token
        )
        ...
        logger.error(
            f"Unexpected error during email login for {credentials.email}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def register_user(user_data: UserRegister, db: Prisma = Depends(get_db)) -> TokenResponse:
    ...
    **Public Registration**: Only CUSTOMER role is allowed.
    ...
    try:
        ...
        logger.error(
            f"Unexpected error during registration for {user_data.email}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
