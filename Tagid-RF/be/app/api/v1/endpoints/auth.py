...
from app.crud.user import get_user_by_email, update_user_google_info, create_user, authenticate_user
...
async def login_with_email(
    ...
    try:
        ...
        logger.error(f"Unexpected error during email login for {credentials.email}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
