...
async def verify_match(
    request: VerifyRequest, encryption: TagEncryptionService = Depends(get_encryption)
):
    """
...
async def decrypt_qr(
    request: DecryptRequest, encryption: TagEncryptionService = Depends(get_encryption)
):
    """
    ...
    if not epc:
        return DecryptResponse(
            success=False, error="Failed to decrypt QR code - invalid or corrupted"
        )
    ...
    if mapping:
        return DecryptResponse(
            success=True, epc=epc, product_id=mapping.productId, container_id=mapping.containerId
        )
    else:
        return DecryptResponse(
            success=True, epc=epc, error="EPC decrypted but no mapping found in database"
        )
...
async def delete_mapping(
    ...
    **Requires**: STORE_MANAGER role or higher.
    """
    try:
        await prisma_client.client.tagmapping.delete(where={"id": mapping_id})
...
async def list_mappings(skip: int = 0, take: int = 50):
    ...
    mappings = await prisma_client.client.tagmapping.find_many(
        skip=skip, take=take, order={"createdAt": "desc"}
    )
