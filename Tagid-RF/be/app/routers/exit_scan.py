...
async def check_exit_scan(
    request: ExitScanRequest,
    ...
    for epc in request.epcs:
        ...
        if not tag:
            ...
            unpaid_items.append(UnpaidItemAlert(
                epc=epc,
    ...
    if unpaid_items:
        ...
        stakeholders = db.query(User).filter(
            User.is_active == True,
            User.role.in_(["ADMIN", "MANAGER", "SELLER"])
        )
...
async def mark_tags_as_unpaid(
    ...
    return {
        "message": f"Marked {updated} tags as unpaid",
        "updated_count": updated
    }
