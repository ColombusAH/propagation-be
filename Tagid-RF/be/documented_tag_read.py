...
def log_frame_breakdown(data: bytes, is_response: bool = False):
    ...
    print(f"   [4] LEN     = 0x{data_len:02X} ({data_len} bytes of data {'+ STATUS' if is_response else ''})")
    offset += 1
    ...
    if is_response and len(data) > offset:
        ...
        if actual_data_len > 0 and len(data) > offset:
            ...
            print(f"   [6-{5+actual_data_len}] DATA = {payload.hex().upper()} ({actual_data_len} bytes)")
