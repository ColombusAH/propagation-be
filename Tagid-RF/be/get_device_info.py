...
def parse_device_info(data: bytes) -> dict:
    ...
    if len(data) < 32:
        return {"error": f"Data too short: {len(data)} bytes", "raw": data.hex().upper()}
    ...
    if len(data) >= offset + 32:
        ...
        rfid_hw = data[offset:offset+32].rstrip(b'\x00').decode('ascii', errors='replace').strip()
        result["rfid_hardware_version"] = rfid_hw
        offset += 32
    
    if len(data) >= offset + 32:
        # RFIDModeName (32 bytes)
        rfid_name = data[offset:offset+32].rstrip(b'\x00').decode('ascii', errors='replace').strip()
        result["rfid_module_name"] = rfid_name
        offset += 32
    
    if len(data) >= offset + 12:
        # RFIDMode_SNCode (12 bytes)
        rfid_sn = data[offset:offset+12].rstrip(b'\x00').decode('ascii', errors='replace').strip()
